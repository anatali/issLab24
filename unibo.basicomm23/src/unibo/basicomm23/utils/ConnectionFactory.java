          ...
          try {
              switch( protocol ){
                case http  : {
                    ...
                    CommUtils.outyellow("    --- ConnectionFactory | create htpp conn host=" + host );
                    return conn;
                }
                case ws   : {
                    Interaction  conn  =  WsConnection.create( host, entry );
                    CommUtils.outyellow("    --- ConnectionFactory | create ws conn:" + host+" entry="+entry);
                    return conn;
                }
                case tcp   : {
                    //entry = portNum
                ...
                case coap   : {
                      CommUtils.outyellow("    --- ConnectionFactory | create coap conn " + host + " entry=" + entry );
                      Interaction  conn  = CoapConnection.create(host, entry);
                      return conn;
                }
                case mqtt   : {
                      CommUtils.outyellow("    --- ConnectionFactory | create mqtt conn " + host + " entry=" + entry );
                      Interaction  conn  =  MqttConnection.create("clientcomm",host,entry);
                      return conn;
                }
                case bluetooth   : {
