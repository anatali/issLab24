 ...
 */
public class MqttConnection extends Connection { //implements Interaction {
...
protected String topicInput = "";
	...
	public static synchronized MqttConnection create(   ) {
		if( mqttSup == null  ) mqttSup = new MqttConnection(   );
		return mqttSup;
	}
...
//			options.setWill("unibo/clienterrors", "crashed".getBytes(), 2, true);
			...
			if( trace ) ColorsOut.out("MqttConnection | connected client " + client.getClientId() + " to broker " + brokerAddr );
   ...
   //Introduced for unibo.qakactor22
	...
	public void subscribe ( String topic, IApplMsgHandlerMqtt handler) {
		...
		if( trace ) ColorsOut.outappl(clientid + " | MqttConnection handler="+
	...
	public void subscribe(String clientid, String topic, MqttCallback callback) {
		try {
			...
			if( trace ) ColorsOut.outappl("subscribed " + client + " topic=" + topic
	...
	public void publish(String topic, String msg, int qos, boolean retain) {
		if( trace ) CommUtils.outyellow("MqttConnection  | publish " + msg + " on " + topic );
		...
		try {
			//CommUtils.outyellow("MqttConnection  | publish topic=" + topic + " msg=" + msg + " client=" + client );
			message.setPayload(msg.toString().getBytes());		 
			client.publish(topic, message);
			//ColorsOut.outappl("MqttConnection  | publish-DONE on topic=" + topic, ColorsOut.CYAN );
		} catch (MqttException e) {
			CommUtils.outred("MqttConnection  | publish Error "  + client.getClientId() + " " + e.getMessage());
		}
	}
...
//----------------------------------------------------	
	...
	public void forward(String msg) throws Exception {
		if( trace ) ColorsOut.outappl("forward " + msg , ColorsOut.BLUE);
		try{
	...
	public String request(String msg) throws Exception { //msg should contain the name of the sender
		if( trace ) ColorsOut.outappl("... request " + msg + " by clientid=" + clientid + " support=" + this, ColorsOut.CYAN);
...
//Preparo per ricevere la risposta
		...
		if( trace ) CommUtils.outred("request answerTopicName="+answerTopicName );
...
//		 	answer = msgAnswer.msgContent(); 		
	...
	public String receiveMsg()   {
		if( trace ) CommUtils.outyellow("MqttConnection | receiveMsg ... blockingQueue=" + blockingQueue.size()  );
		String answer = "data unknown ...";
 		try {
			...
			if( trace ) ColorsOut.outappl("MqttConnection | receiveMsg answer=" + answer , ColorsOut.CYAN);
		}catch( Exception e){
...
//	}
	...
	public void cleartopic(String topic){
		try {
			...
			CommUtils.outblue("MqttConnection  | cleartopic m=" + msg);
			publish(topic, msg,0,true);
		} catch ( Exception e) {
			CommUtils.outred("MqttConnection  | cleartopic Error:" + e.getMessage());
		}
	}
