...
public class LifeController {
    ...
    /*
    public void startStopClicked(ActionEvent event, Button startStop) {
        CommUtils.outyellow("LifeController | startStopClicked " + event.getSource());
        AnimationTimer runAnimation = new AnimationTimer() {
            private long lastUpdate = 0;

            @Override
            public void handle(long now) {
                 if ((now - lastUpdate) >= TimeUnit.MILLISECONDS.toNanos((long) generationTime)) {
                    life.play();
                    lastUpdate = now;
                }
        ...
        if (event.getSource().toString().contains("Start")) {
            runAnimation.start();
            startStop.setText("Stop");
        } else {
            runAnimation.stop();
