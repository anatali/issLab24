...
/*
 * ===========================================================================
 * TestPingPong24SingleCtxWithReferee
 * ===========================================================================
 */
public class TestPingPong24SingleCtxWithReferee {
...
public static void activateSystemUsingGradle() { 
	Thread th = new Thread(){
		public void run(){
			try {
				CommUtils.outmagenta("TestPingPong24SingleCtxWithReferee activateSystemUsingGradle ");
				Process p = Runtime.getRuntime().exec("./gradlew.bat runAll");
				showOutput(p,ColorsOut.BLACK);
			} catch ( Exception e) {
				CommUtils.outred("TestPingPong24SingleCtxWithReferee activate ERROR " + e.getMessage());
			}
		}
	};
	th.start();
...
public static void activateSystemUsingDeploy() { 
	Thread th = new Thread(){
		public void run(){
			try {
				CommUtils.outmagenta("TestPingPong24SingleCtxWithReferee activateSystemUsingDeploy ");
 ...
 */
	...
	public static void activate() {
		CommUtils.outmagenta("TestPingPong24SingleCtxWithReferee activate ");
 ...
 */
	...
	public void testPingPongSystem() {
		IApplMessage req  = CommUtils.buildRequest( "tester", "info", "info(X)", "referee");
 		try {
 			Thread.sleep(1000);  //wait since gradle ...
			...
			assertEquals(answer, "obsinfo(4)");
		} catch (Exception e) {
			CommUtils.outred("testPingPongSystem ERROR " + e.getMessage());
